...
public class BayTekActivity extends ActionBarActivity implements OnFragmentInteractionListener, DQBaytekMachine.DQBaytekMachineListenerInterface {
    ...
    {
        USERS, DEVICES, USER_DETAIL, DEVICE_DETAIL, ADMIN, ADMIN_DETAIL
    }
    ...
    private AdminDetailFragment adminDetailFragment;
    ...
    public void onFragmentInteraction(FragmentType fragmentType, Object model) {
        ...
        else if (fragmentType == FragmentType.ADMIN) {
            AdminModel device = (AdminModel) model;
            mLayout.setPanelState(SlidingUpPanelLayout.PanelState.EXPANDED);

            // Create an instance of editorFrag
            adminDetailFragment = AdminDetailFragment.newInstance(device, this);


            // add fragment to the fragment container layout
            getSupportFragmentManager().beginTransaction().add(R.id.fragment_container, adminDetailFragment).addToBackStack("deviceDetail").commit();
        }
    }
